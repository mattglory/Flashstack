...
| D5 | `deployments/default.mainnet-plan.yaml` describes a gen-1 publish from `SP3TGRVG…`, and contains none of the live system. `/deployments/` is CODEOWNERS-protected, so it reads as authoritative. | file contents vs `find contracts` | **OPEN.** Re-confirmed verbatim 2026-09-16: the file publishes 13 gen-1 contracts from `SP3TGRVG…` at `epoch 2.5`, including `contracts/snp-flashstack-receiver.clar` and `contracts/snp-flashstack-receiver-v3.clar`. *(Update 2026-09-21: both files now exist in the repo, see D7, so the plan's paths resolve. The plan is still a gen-1 publish and still not the live system, so this stays open.)* Needs Matt's decision on disposition (delete / archive / replace) — see §7. | `Flashstack-ajv.7.2` |
