...
**Evidence date:** 2026-09-17 (repo facts re-read on this date)
...
contracts) from *remap-to-testnet-equivalent* (the SIP-010 standard trait). Today it
...
### 5.3 `flashstack-pool-v3` needs epoch 4.0 on testnet

`flashstack-pool-v3` is the only contract in the project at `clarity_version = 6`,
`epoch = "4.0"` — it uses `as-contract?` and `current-contract` (design doc §11).
Every other current-gen contract is `clarity_version = 3`, `epoch = "3.0"`.

**Verify before attempting:** that testnet is at Epoch 4.0 and accepts a
`clarity-version 6` publish. If it is not, pool-v3 cannot be staged at all yet and
that fact — not a workaround — is the finding.
...
| # | Action | Blocked on |
|---|---|---|
| 1 | Confirm testnet epoch/clarity-version support for pool-v3 (§5.3) | Nothing — public API read |
| 2 | Decide the sBTC mock question (§5.2) | Project owner |
| 3 | Extend the patcher to handle remap-vs-rewrite (§5.1) | Decision on 1 and 2 |
| 4 | Write `deployments/testnet-current-gen-plan.yaml` | 1–3 |
